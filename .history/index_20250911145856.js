    ...
    try {
        ...
        app.patch("/courses/:id", async (req, res) => {
            try {
                ...
                // MongoDB native driver use
                const course = await courseCollection.findOneAndUpdate(
                    { _id: new ObjectId(id) }, // ObjectId এ convert
                    { $set: { status } },
                    { returnDocument: "after" } // আপডেট হওয়া document ফেরত দেয়
                );

                if (!course.value) return res.status(404).json({ message: "Course not found" });
